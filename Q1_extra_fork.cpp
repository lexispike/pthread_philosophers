 ...
 *
 */
void print_state(int thread_num, string state, string action, string fork1, string fork2) {
    if (!all_philosophers_done()) { // check if all the philosophers are done or not, or else we output bad data
        ...
        if (state == "thinking") {
            cout << "- waiting for middle fork OR " << endl;
        }
        cout << "- " << action << " fork: " << fork1 << endl;
        cout << "- " << action << " fork: " << fork2 << endl << endl;
    ...
    while (!all_philosophers_done()) {
        ...
        if (pthread_mutex_trylock(&locks[get_left_fork(thread_num)]) == 0) {
            if (pthread_mutex_trylock(&locks[get_right_fork(thread_num)]) == 0) {
                print_state(thread_num, "eating", "has", to_string(get_left_fork(thread_num)), to_string(get_right_fork(thread_num)));
                p_eaten[thread_num] = 1;

                sleep(1); // simulate some eating time
                pthread_mutex_unlock(&locks[get_right_fork(thread_num)]);
            } else { // try the middle fork
                if (pthread_mutex_trylock(&middle_lock) == 0) {
                    print_state(thread_num, "eating", "has", to_string(get_left_fork(thread_num)), "middle");
                    p_eaten[thread_num] = 1;

                    sleep(1); // simulate some eating time
                    pthread_mutex_unlock(&middle_lock);
                } else {
                    print_state(thread_num, "thinking", "waiting for", to_string(get_left_fork(thread_num)), to_string(get_right_fork(thread_num)));
                    p_thought[thread_num] = 1;
                }
            }
            pthread_mutex_unlock(&locks[get_left_fork(thread_num)]);
        } else { // try the right fork with the middle fork
            if (pthread_mutex_trylock(&locks[get_right_fork(thread_num)]) == 0) {
                if (pthread_mutex_trylock(&middle_lock) == 0) {
                    print_state(thread_num, "eating", "has", to_string(get_right_fork(thread_num)), "middle");
                    p_eaten[thread_num] = 1;
...
int main(int argc, char* argv[]) {
    ...
    if (pthread_mutex_init(&middle_lock, NULL) != 0) {
        cout << "Error: failed to initialize mutex" << endl;
        return 1;
    }
    ...
    pthread_mutex_destroy(&middle_lock);
